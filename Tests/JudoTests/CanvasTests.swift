...
//        XCTAssertEqual([0.5, 0.7, 0.4, 0.8], comps("hsla(100, 33%, 55%, 0.8)"))
    ...
    func testFontParsing() throws {
        ...
        func check(font fontShorthand: String, parses expectedFont: String, size: Double = 12.0) throws {
            #if canImport(AppKit)
            guard let parsedFont = CSS.parseFontStyleWebKit(css: fontShorthand) else {
        ...
        try check(font: "12px Serif", parses: "Times Roman")
        try check(font: "12px Sans-Serif", parses: "Helvetica")
        try check(font: "12px Cursive", parses: "Apple Chancery")
        try check(font: "12px Fantasy", parses: "Papyrus")
        try check(font: "12px Monospace", parses: "Courier")

        try check(font: "12px X, MONOSPACE", parses: "Courier")
        try check(font: "12px MONOSPACE, X", parses: "Courier")
        try check(font: "19px X, FANTASY", parses: "Papyrus", size: 19)
        try check(font: "19px FANTASY, X", parses: "Papyrus", size: 19)

        try check(font: "12px/14px sans-serif", parses: "Helvetica")
        try check(font: "1.2em \"Fira Sans\", sans-serif", parses: "Helvetica", size: 14.4)
        try check(font: "italic small-caps bold 16px/2 cursive", parses: "Apple Chancery", size: 16.0)
        try check(font: "small-caps bold 24px/1 sans-serif", parses: "Helvetica Bold", size: 24.0)
        try check(font: "caption", parses: "System Font Regular", size: 13)
        try check(font: "80% sans-serif", parses: "Helvetica", size: 9.6)
        try check(font: "bold italic large serif", parses: "Times Bold Italic", size: 14)

        // https://github.com/web-platform-tests/wpt/blob/master/css/css-fonts/variations/font-shorthand.html
        try check(font: "700.5 24px Arial", parses: "Arial Bold", size: 24)
        try check(font: "oblique 45deg 24px Arial", parses: "Arial Italic", size: 24)
        try check(font: "oblique 24px Arial", parses: "Arial Italic", size: 24)
        try check(font: "oblique 50 24px Arial", parses: "Arial Italic", size: 24)
        try check(font: "oblique 500 24px Arial", parses: "Arial Italic", size: 24)
        try check(font: "oblique 45deg 500 24px Arial", parses: "Arial Italic", size: 24)

        // these might be dependent on local system settings, so we may need to add in some fuzzy matching…
        try check(font: "caption", parses: "System Font Regular", size: 13.0)
        try check(font: "icon", parses: "System Font Regular", size: 13.0)
        try check(font: "menu", parses: "System Font Regular", size: 13.0)
        try check(font: "message-box", parses: "System Font Regular", size: 13.0)
        try check(font: "small-caption", parses: "System Font Regular", size: 11.0)
        try check(font: "status-bar", parses: "System Font Regular", size: 10.0)
    }
    #endif // canImport(CoreGraphics)
}
